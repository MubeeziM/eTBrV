...
public sealed class TBPatientsController : ControllerBase
{
        ...
        [FromQuery] string  mode        = "missed")
    {
        ...
        try
        {
            ...
            // 5-month: DS-TB only, COALESCE(DateRxStarted, RegDate) as base, Mon5LabResultID check
            int sputum5;
            {
                string s5Base = "COALESCE(p.DateRxStarted, p.RegDate)";
                string s5Dl   = $"DATEDIFF(DAY,{s5Base},GETDATE()) - 140 - (CASE DATENAME(WEEKDAY,DATEADD(DAY,140,{s5Base})) WHEN 'Saturday' THEN 2 WHEN 'Sunday' THEN 1 ELSE 0 END)";
                string s5Mode = missed ? $"AND ({s5Dl}) > 0 AND ({s5Dl}) <= 28" : $"AND ({s5Dl}) <= 0";
                var sql5 = $"""
                    SELECT COUNT(*)
                    FROM PtDetailsT p
                    LEFT JOIN PtFollowUpT fu ON p.PtDetailsTID = fu.PtDetailsTID AND fu.Deleted = 0
                    ...
                    WHERE p.Deleted = 0
                      ...
                      {s5Mode}
                    """;
                await using var cmd5 = new SqlCommand(sql5, conn);
                AddFacParams(cmd5, facPrms);
                var r5 = await cmd5.ExecuteScalarAsync();
                sputum5 = r5 == null || r5 == DBNull.Value ? 0 : Convert.ToInt32(r5);
            }
            // 6-month: DS-TB only, COALESCE(Mon5Date+28, DateRxStarted+168), Mon6LabResultID check
            int sputum6;
            {
                string s6Ideal = "COALESCE(DATEADD(DAY,28,fu.Mon5Date),DATEADD(DAY,168,p.DateRxStarted))";
                string s6Dl    = $"DATEDIFF(DAY,{s6Ideal},GETDATE()) - (CASE DATENAME(WEEKDAY,{s6Ideal}) WHEN 'Saturday' THEN 2 WHEN 'Sunday' THEN 1 ELSE 0 END)";
                string s6Mode  = missed ? $"AND ({s6Dl}) > 0 AND ({s6Dl}) <= 56" : $"AND ({s6Dl}) <= 0";
                var sql6 = $"""
                    SELECT COUNT(*)
                    FROM PtDetailsT p
                    LEFT JOIN PtFollowUpT fu ON p.PtDetailsTID = fu.PtDetailsTID AND fu.Deleted = 0
                    ...
                    WHERE p.Deleted = 0
                      ...
                      {s6Mode}
                    """;
                await using var cmd6 = new SqlCommand(sql6, conn);
                AddFacParams(cmd6, facPrms);
                var r6 = await cmd6.ExecuteScalarAsync();
                sputum6 = r6 == null || r6 == DBNull.Value ? 0 : Convert.ToInt32(r6);
            }
            ...
            int sputum8;
            {
                ...
                var sql8 = $"""
                    ...
                    WHERE p.Deleted = 0
                      ...
                      {s8Mode}
                    """;
                await using var cmd8 = new SqlCommand(sql8, conn);
                AddFacParams(cmd8, facPrms);
        ...
        [FromQuery] string  category    = "2month")
    {
        ...
        switch (category)
        {
            ...
            case "5month":
            {
                string base5    = "COALESCE(p.DateRxStarted, p.RegDate)";
                string wkAdj5   = $"CASE DATENAME(WEEKDAY,DATEADD(DAY,140,{base5})) WHEN 'Saturday' THEN 2 WHEN 'Sunday' THEN 1 ELSE 0 END";
                string dl5      = $"DATEDIFF(DAY,{base5},GETDATE()) - 140 - ({wkAdj5})";
                string modeFilter5 = missed
                    ? $"AND ({dl5}) > 0 AND ({dl5}) <= 28"
                    : $"AND ({dl5}) <= 0";
                querySql = $"""
                    SELECT {baseColsSputum},
                           {dl5} AS DaysLate
            ...
            case "6month":
            {
                string ideal6    = "COALESCE(DATEADD(DAY,28,fu.Mon5Date),DATEADD(DAY,168,p.DateRxStarted))";
                string wkAdj6    = $"CASE DATENAME(WEEKDAY,{ideal6}) WHEN 'Saturday' THEN 2 WHEN 'Sunday' THEN 1 ELSE 0 END";
                string dl6       = $"DATEDIFF(DAY,{ideal6},GETDATE()) - ({wkAdj6})";
                string modeFilter6 = missed
                    ? $"AND ({dl6}) > 0 AND ({dl6}) <= 56"
                    : $"AND ({dl6}) <= 0";
                querySql = $"""
                    SELECT {baseColsSputum},
                           {dl6} AS DaysLate
            ...
            case "2month":
            {
                string extra2   = "AND (fu.PtFollowUpTID IS NULL OR fu.Mon2Date IS NULL OR COALESCE(fu.Mon2LabResultID,0) IN (0,3,7)) AND (fu.PtFollowUpTID IS NULL OR COALESCE(fu.Mon3LabResultID,0) IN (0,3,7))";
                string wkAdj2   = "CASE DATENAME(WEEKDAY,DATEADD(DAY,56,p.DateRxStarted)) WHEN 'Saturday' THEN 2 WHEN 'Sunday' THEN 1 ELSE 0 END";
                string dl2      = $"DATEDIFF(DAY,p.DateRxStarted,GETDATE()) - 56 - ({wkAdj2})";
                string modeFilter2 = missed
                    ? $"AND ({dl2}) > 0 AND ({dl2}) <= 28"
                    : $"AND ({dl2}) <= 0";
                querySql = $"""
                    SELECT {baseColsSputum},
                           {dl2} AS DaysLate
    ...
    public async Task<IActionResult> GetQualityCounts([FromQuery] int[]? facilityIds = null)
    {
        ...
        try
        {
            // Run all 12 COUNT queries in parallel — each on its own connection.
            // Wall-clock time = max(slowest query) rather than sum(all queries).
            // (The earlier "sequential single-connection" workaround was a red
            // herring; the real bug was duplicate @FId0 parameter names when both
            // facPrms and facDPrms were added to the same SqlCommand.)
            async Task<int> Cnt(string sql, List<(string Name, int Value)>? extraPrms = null)
            {
                await using var conn = new SqlConnection(_connectionString);
                await conn.OpenAsync();
                await using var cmd = new SqlCommand(sql, conn) { CommandTimeout = 55 };
                AddFacParams(cmd, facPrms);
                if (extraPrms != null) AddFacParams(cmd, extraPrms);
                cmd.Parameters.AddWithValue("@MinYear", minYear);
                var r = await cmd.ExecuteScalarAsync();
                return r == null || r == DBNull.Value ? 0 : Convert.ToInt32(r);
            }
            ...
            return Ok(new {
                all          = tAll.Result,
                duplicates   = tDuplicates.Result,
                skipped      = tSkipped.Result,
                sametbno     = tSametbno.Result,
